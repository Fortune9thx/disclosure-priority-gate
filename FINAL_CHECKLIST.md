...
- [x] Redeployed (1.1.0, steward-review fix) to GenLayer Bradbury testnet: `0x6E4c3445bE1b2ae0EA4EC73FfDAB93Fc7a6dA2f4`, deploy tx `0xff062c79c252171c0a9ea4a873e1c37901e3d99f7964cda7dc5dfdd747a3cc45` (`ACCEPTED`/`FINISHED_WITH_RETURN`), confirmed readable
- [x] A real, live regression of the exact steward-described scenario run against 1.1.0 (register program -> submit two genuinely distinct reports -> challenge -> evaluate) -- real, unscripted consensus correctly judged `DISTINCT`, and the report was confirmed still `"pending"` afterward, not `"confirmed_original"`. `confirm_report` also confirmed live to correctly reject before the window elapses. See `docs/DESIGN.md#live-verification`.
